Question Link: [https://leetcode.com/problems/minimum-cost-to-convert-string-i](https://leetcode.com/problems/minimum-cost-to-convert-string-i)
...
private:
    ...
    buildConversionGraph(const std::vector<char>& original,
                         const std::vector<char>& changed,
                         const std::vector<int>& cost) {
        std::vector<std::vector<int>> graph(CHAR_COUNT,
                                            std::vector<int>(CHAR_COUNT, INF));
    ...
    long long
    computeTotalConversionCost(const std::string& source,
